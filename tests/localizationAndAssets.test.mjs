...
test("ultimate button triggers a 1.5 second cut-in illustration overlay", async () => {
  ...
  assert.match(styles, /\.ultimate-cutscene__image\s*{[^}]*top:\s*44%/s);
  assert.match(styles, /\.ultimate-cutscene__image\s*{[^}]*height:\s*min\(68%,\s*570px\)/s);
  ...
  assert.match(styles, /transform:\s*translate\(-50%,\s*-51%\)\s*scale\(0\.94\)/);
  assert.doesNotMatch(styles, /transform:\s*translate\(-50%,\s*-50%\)\s*scale\(1\)/);
